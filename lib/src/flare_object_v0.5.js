const FlareObject = (function() {
	...
	FlareObject.prototype.setActor = function(actor) {
		...
		if (actorInstance) {
			...
			if (actorInstance._Animations.length) {
				...
				this._Animation1 = actorInstance._Animations[0]; // console.log(this._Animation1);
				this._AnimationInstance1 = new Flare.AnimationInstance(this._Animation1._Actor, this._Animation1); //console.log(this._AnimationInstance1);
				...
				//console.log(this._AnimationInstance1,this._AnimationInstance2);

				if (!this._AnimationInstance1) {
	...
	FlareObject.prototype.advanceActor = function( elapsed ) {
		...
		if (actorArtboardInstance) {
			...
			actorArtboardInstance.advance(elapsed);
			//actor.root.x += 0.1;
		}
	};
	 ...
	 */
	FlareObject.prototype.updateAnimation = function( { elapsedAnimation1 = 1.0, elapsedAnimation2 = 1.0 } ) {
		const actor = this._Actor;
		const actorArtboardInstance = this._ActorInstance;

		const animationInstance1 = this._AnimationInstance1;
		const animationInstance2 = this._AnimationInstance2;

		//if ( actor && actorArtboardInstance && animationInstance1 != '' && animationInstance2 != '' ) {
			...
			if ( animationInstance1 ) { 
				animationInstance1.time = elapsedAnimation1;
				animationInstance1.apply(actorArtboardInstance, 1.0);
			}

			this.advanceActor(elapsedAnimation1);

			_Draw(this, this._Graphics);

			if ( animationInstance2 ) { 
				animationInstance2.time = elapsedAnimation2;
				animationInstance2.apply(actorArtboardInstance, 1.0);
			}

			this.advanceActor(elapsedAnimation2);
